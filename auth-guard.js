...
(function () {
  ...
  // 共有リンク(?share=...)は顧問向けの公開ページのため、ログインを要求しない。
  // 表示できるのはそのURLのUUIDに対応する1件のみ（DB側の関数でスコープを限定）。
  if (new URLSearchParams(location.search).get('share')) {
    window.__appAuth = { session: null, ready: true };
    return;
  }
    ...
    overlay.innerHTML = `
      <div style="background:#fff;padding:32px 28px;border-radius:16px;box-shadow:0 8px 32px rgba(15,23,42,0.12);width:100%;max-width:400px;border:1px solid #e2e8f0">
        <div style="text-align:center;margin-bottom:24px">
          ...
          <div style="font-size:20px;font-weight:700;color:#2563eb;letter-spacing:0.02em">顧問先管理アプリ</div>
